...
def compare_dihedrals(traj_xyz_ref, traj_xyz_ancestral, traj_xyz_mwg ,topology, filename, bins = 40, rng = [[-np.pi,np.pi],[-np.pi, np.pi]],psi_inds = [6, 8, 14, 16], phi_inds = [4, 6, 8, 14]):
...
def compare_rogs(traj_xyz_ref, traj_xyz_anc, traj_xyz_mwg, topology, filename, density = True, xlims = [0.2, 0.28], histtype = 'step'):
